...
X_data_2 <- data.matrix(subset(data_frame, select = -  c(outcome_column)))    # this was changed from as.matrix as was getting an error
...
#f <- as.formula(paste("outcome_column ~", paste(n[!n %in% "outcome_column"], collapse = " + ")))    #pastes all factors separated by a +
#f <- as.formula(paste("outcome_column ~", "HR_above + HR_below + RR_above + RR_below + FiO2 + inverse_SATs")) #small scale test

nnames <- colnames(X_data_2)

f <- as.formula(paste("outcome_column ~", paste(nnames[!nnames %in% "outcome_column"], collapse = " + ")))
...
nn_test_X <- data.frame(subset(ML_data_test, select = - c(outcome_column)))
nn_test_Y <- data.frame(ML_data_test[,"outcome_column"])
